...
function tmrco_core_plugin_infomation( $res, $action, $args ){
	...
	if( false == $remote = get_transient( 'tmrco_core_update_' . $plugin_slug ) ) {
		$remote = wp_remote_get( 'https://demo.rimtm.com/wordpress/plugins/tmrco/update.json', array(
			'timeout' => 10,
			'headers' => array(
...
function tmrco_core_plugin_push_update( $transient ){
	...
	if( false == $remote = get_transient( 'tmrco_core_upgrade_tmrco-core' ) ) {
		...
		if ( !is_wp_error( $remote ) && isset( $remote['response']['code'] ) && $remote['response']['code'] == 200 && !empty( $remote['body'] ) ) {
			set_transient( 'tmrco_core_upgrade_tmrco-core', $remote, 14400 ); // 4 hours cache
		}
	}
	...
	if( $remote ) {
		...
		if( $remote && version_compare( tmrco_CORE_VERSION, $remote->new_version, '<' )
			...
			$res = new stdClass();
			$res->slug = 'tmrco-core';
			$res->plugin = 'tmrco-core/tmrco-core.php';
			$res->new_version = $remote->new_version;
...
function tmrco_core_after_update( $upgrader_object, $options ) {
	if ( $options['action'] == 'update' && $options['type'] === 'plugin' )  {
		delete_transient( 'tmrco_core_upgrade_tmrco-core' );
	}
}
