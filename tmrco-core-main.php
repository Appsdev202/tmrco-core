<?php
/*
 * Plugin Name:       tmrco Core 
 * Plugin URI:        https://www.themeum.com/
 * Description:       tmrco Core Plugin
 * Version: 		  1.1.1
 * Author:            Themeum.com
 * Author URI:        https://themeum.com/
 * Text Domain:       tmrco-core 
...
function tmrco_core_language_load(){
    load_plugin_textdomain( 'tmrco-core-main', false,  basename(dirname(__FILE__)).'/languages/' );
}
...
if(!function_exists('tmrco_core_style')):
    function tmrco_core_style(){
        # CSS
        //wp_enqueue_style('tmrco-core-main-css',plugins_url('assets/css/blocks.style.build.css',__FILE__));
        # JS
...
function tmrco_core_load_admin_assets() {
    wp_enqueue_script( 'tmrco-core-main-admin', plugins_url('assets/js/admin.js', __FILE__), array('jquery') );
...
if ( ! function_exists( 'tmrco_core_featured_image_urls' ) ) {
    function tmrco_core_featured_image_urls( $object, $field_name, $request ) {
        ...
        return array(
            ...
            'portrait'      => is_array( $image ) ? wp_get_attachment_image_src( $object['featured_media'], 'tmrco-core-main-portrait', false ) : '',
            'portraitab'    => is_array( $image ) ? wp_get_attachment_image_src( $object['featured_media'], 'tmrco-course-tab', false ) : '',
            'thumbnail'     => is_array( $image ) ? wp_get_attachment_image_src( $object['featured_media'], 'tmrco-core-main-thumbnail', false ) : '',
        );
    }
}
...
if ( ! function_exists( 'tmrco_core_blog_posts_image_sizes' ) ) {
    function tmrco_core_blog_posts_image_sizes() {
        add_image_size( 'tmrco-core-main-portrait', 700, 870, true );
        add_image_size( 'tmrco-course-tab', 322, 300, true );
        add_image_size( 'tmrco-core-main-thumbnail', 140, 100, true );
    }
            ...
            if(have_posts()){
                while(have_posts()){
                    ...
                    if ($is_purchasable && $price){
                        ...
                        $price_data = '<div class="price">'.__("Free", "tmrco-core-main").'</div>';
                    }
                    ...
                    if (rimcour_utils()->is_course_purchasable()) {
                        ...
                        $cart_data = '<a href="'.esc_url(get_the_permalink(get_the_ID())).'" class="btn btn-classic btn-no-fill">'.__('Enroll Now','tmrco-core-main').'</a>';
                    }
                    ...
                    $featured_images = array(
                        'full'      => is_array( $image ) ? $image : '',
                        'portrait'      => is_array( $image ) ? wp_get_attachment_image_src(get_post_thumbnail_id( get_the_ID()), 'tmrco-core-main-portrait', false ) : '',
                        'portraitab'    => is_array( $image ) ? wp_get_attachment_image_src(get_post_thumbnail_id( get_the_ID()), 'tmrco-course-tab', false ) : '',
                        'thumbnail'     => is_array( $image ) ? wp_get_attachment_image_src(get_post_thumbnail_id( get_the_ID()), 'tmrco-core-main-thumbnail', false ) : '',
                    );
